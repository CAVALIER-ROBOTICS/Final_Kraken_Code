...
import frc.robot.subsystems.Liberderry.SteerConfiguration;
import frc.robot.subsystems.Liberderry.SteerController;
import frc.robot.subsystems.Liberderry.SteerControllerFactory;
import static frc.robot.subsystems.Liberderry.ctre.CtreUtils.checkCtreError;
    ...
    private static class ControllerImplementation implements SteerController {
        ...
        public void setReferenceAngle(double referenceAngleRadians) {
            ...
            if (motor.getSelectedSensorVelocity()
                    * motorEncoderVelocityCoefficient < ENCODER_RESET_MAX_ANGULAR_VELOCITY && Math.abs(motor.getSelectedSensorPosition()) < 10) {
                if (++resetIteration >= ENCODER_RESET_ITERATIONS) {
                    resetIteration = 0;
                    double absoluteAngle = absoluteEncoder.getAbsoluteAngle();
                    // motor.setSelectedSensorPosition(absoluteAngle / 2048);
